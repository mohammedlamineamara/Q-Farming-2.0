...
// ─── Harvests / Production ────────────────────────────────────
export const harvests = mysqlTable(
  "harvests",
  {
    id: bigint("id", { mode: "number", unsigned: true })
      .autoincrement()
  ...
  (table) => [
    index("harvests_user_field_idx").on(table.userId, table.fieldId),
  ]
);
...
export const sales = mysqlTable(
  ...
  (table) => [
    index("sales_user_harvest_idx").on(table.userId, table.harvestId),
  ]
);
...
export const inventoryItems = mysqlTable(
  ...
  (table) => [
    index("inventory_items_user_id_idx").on(table.userId),
  ]
);
...
export const sensors = mysqlTable(
  ...
  (table) => [
    index("sensors_user_field_idx").on(table.userId, table.fieldId),
  ]
);
...
export const aiInsights = mysqlTable(
  ...
  (table) => [
    index("ai_insights_user_cat_idx").on(table.userId, table.category),
  ]
);
...
export const calendarEvents = mysqlTable(
  ...
  (table) => [
    index("calendar_events_user_date_idx").on(table.userId, table.eventDate),
  ]
);
